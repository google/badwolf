...
// AddTriplesBenchmark creates the benchmark
func AddTriplesBenchmark(ctx context.Context, st storage.Store) ([]*runtime.BenchEntry, error) {
	bFactors := []int{2, 200}
	...
	for idx, g := range gs {
		for _, s := range sizes {
			...
			ids = append(ids, fmt.Sprintf("branch_factor=%04d, size=%07d", bFactors[idx], s))
	...
	for i, max := 0, len(ids); i < max; i++ {
		for idxReps, r := range reps {
			var g storage.Graph
			gID := fmt.Sprintf("add_%s_r%d_i%d", gids[i], i, idxReps)
			data := trplSets[i]
